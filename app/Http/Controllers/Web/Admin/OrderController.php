...
class OrderController extends Controller
{
    ...
    {
        $orders = Order::query()->with(['tableSession.restaurantTable', 'orderItems.product'])->when($request->status, fn ($query, $status) => $query->where('status', $status))->latest()->get();
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($type === OrderType::TABLE && empty($validated['table_id'])) {
            throw ValidationException::withMessages(['table_id' => ['Selecciona una mesa para un pedido en mesa.']]);
        }
        if ($type === OrderType::TAKEAWAY && ! empty($validated['table_id'])) {
            throw ValidationException::withMessages(['table_id' => ['Un pedido para llevar no puede tener una mesa asociada.']]);
        }
        ...
        $order = DB::transaction(function () use ($validated, $type) {
            ...
            if ($type === OrderType::TABLE) {
                ...
                if ($table->status === TableStatus::CLEANING) {
                    throw ValidationException::withMessages(['table_id' => ['La mesa seleccionada no está disponible para recibir pedidos.']]);
                }
                ...
                if (! $tableSession) {
                    $tableSession = TableSession::create(['restaurant_table_id' => $table->id, 'status' => TableSessionStatus::Active, 'started_at' => now()]);
                }
    ...
    {
        $order->load(['tableSession.restaurantTable', 'orderItems.product', 'statusHistories.changedBy', 'handledBy', 'deliveredBy']);
    ...
    public function updateStatus(Request $request, Order $order): RedirectResponse
    {
        ...
        $allowedValues = array_map(fn (OrderStatus $status) => $status->value, $allowedTransitions[$currentStatus->value] ?? []);
        if (! in_array($newStatus->value, $allowedValues, true)) {
            throw ValidationException::withMessages(['status' => ["No se puede cambiar el pedido de {$currentStatus->value} a {$newStatus->value}."]]);
        }
        DB::transaction(function () use ($order, $currentStatus, $newStatus) {
            $order->update(['status' => $newStatus]);
            $order->statusHistories()->create(['previous_status' => $currentStatus->value, 'new_status' => $newStatus->value, 'changed_by_user_id' => Auth::id(), 'changed_at' => now()]);
        });
