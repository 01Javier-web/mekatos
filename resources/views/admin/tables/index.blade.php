...
<div class="page-shell tables-page">
    <div class="page-heading">
        <div>
            ...
            <h2>Mesas</h2>
            <p class="tables-description">Gestiona las 41 mesas, sus estados y los accesos del menú por QR.</p>
        </div>
...
<div id="table-modal" class="table-modal" hidden aria-hidden="true">
    ...
    <section class="table-dialog" role="dialog" aria-modal="true" aria-labelledby="table-dialog-title">
        ...
        <div class="table-detail-grid">
            ...
            <form id="table-release-form" method="POST" action="#" onsubmit="return confirm('¿Marcar esta mesa como libre? Esto cerrará una sesión vacía si existe. Si la mesa tiene pedidos, la acción será rechazada.')">@csrf<button type="submit" class="table-detail-action table-detail-release"><span class="action-icon">✓</span><span><strong>Marcar como libre</strong><small>Liberar una mesa abandonada</small></span></button></form>
...
@media(max-width:1180px){.table-grid{grid-template-columns:repeat(5,minmax(0,1fr))}}@media(max-width:900px){.table-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:700px){.tables-panel-header{align-items:flex-start}.table-tools{width:100%;display:grid;grid-template-columns:1fr}.admin-search{width:100%}.table-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;padding-bottom:6px}.table-card{min-height:90px;padding:12px 6px}.table-card-number{font-size:.92rem}.table-card-status{font-size:.68rem}.table-detail-grid{grid-template-columns:1fr}.table-dialog{padding:24px 18px}.table-dialog-heading{margin-bottom:18px}}@media(max-width:470px){.table-grid{grid-template-columns:repeat(2,minmax(0,1fr));padding-bottom:5px}.table-card{min-height:86px}.table-dialog{border-radius:16px;padding:24px 16px}.table-detail-action{min-height:64px}}
...
const tableModal=document.getElementById('table-modal'),tableTitle=document.getElementById('table-dialog-title'),tableStatus=document.getElementById('table-dialog-status'),tableCapacity=document.getElementById('table-dialog-capacity'),tableMenu=document.getElementById('table-menu'),tableEdit=document.getElementById('table-edit'),tableRelease=document.getElementById('table-release-form'),tableDelete=document.getElementById('table-delete-form'),tableCopy=document.getElementById('table-copy'),tableQr=document.getElementById('table-qr'),tablePrint=document.getElementById('table-print');let currentTableUrl='';
function openTable(card){const n=card.dataset.tableNumber,name=card.dataset.tableName,status=card.dataset.tableStatus,capacity=card.dataset.tableCapacity;currentTableUrl=card.dataset.qrUrl;tableTitle.textContent=name;tableStatus.textContent=status;tableCapacity.textContent=capacity?`Capacidad: ${capacity} personas`:'';tableMenu.href=currentTableUrl;tableEdit.href=`{{ url('/admin/tables') }}/${n}/edit`;tableRelease.action=`{{ url('/admin/tables') }}/${n}/release`;tableRelease.hidden=status==='Libre';tableDelete.action=`{{ url('/admin/tables') }}/${n}`;tableModal.hidden=false;tableModal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden'}
