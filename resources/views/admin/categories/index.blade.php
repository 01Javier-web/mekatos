...
<div class="page-shell">
    ...
    <section class="panel">
        ...
        <div class="table-wrap"><table class="data-table"><thead><tr><th>Orden</th><th>Nombre</th><th>Descripción</th><th>Estado</th><th class="actions-cell">Disponibilidad</th></tr></thead><tbody id="category-rows">
            @forelse ($categories as $category)
                <tr data-search="{{ strtolower($category->name.' '.($category->description??'')) }}" data-status="{{ $category->is_active?'active':'inactive' }}"><td><strong>{{ $category->sort_order }}</strong></td><td><strong>{{ $category->name }}</strong></td><td>{{ $category->description ?: 'Sin descripción' }}</td><td><span class="status {{ $category->is_active?'status-active':'status-inactive' }}">{{ $category->is_active?'Activa':'Inactiva' }}</span></td><td class="actions-cell"><form method="POST" action="{{ route('admin.categories.availability',$category) }}" class="inline-form">@csrf @method('PUT')<button class="button button-small {{ $category->is_active ? 'button-danger' : 'button-primary' }}" type="submit">{{ $category->is_active ? 'Deshabilitar' : 'Habilitar' }}</button></form></td></tr>
            @empty<tr><td colspan="5" class="empty-state"><h3>No hay categorías</h3><p>Las categorías se crean y administran desde Configuración.</p><a class="button button-primary" href="{{ route('admin.categories.create') }}">Nueva categoría</a></td></tr>@endforelse
        </tbody></table></div>
