...
<div class="page-shell">
    <div class="page-heading"><div><span class="eyebrow">Operación</span><h2>Pedidos</h2><p>Consulta y gestiona los pedidos del restaurante.</p></div></div>
    ...
    <section class="panel">
        ...
        <div class="table-wrap"><table class="data-table">
            <thead><tr><th>Pedido</th><th>Mesa</th><th>Productos</th><th>Total</th><th>Estado</th><th class="actions-cell">Acciones</th></tr></thead>
            <tbody>
            @forelse ($orders as $order)
                <tr>
                    <td><strong>#{{ $order->id }}</strong><small>{{ $order->created_at?->format('d/m/Y H:i') }}</small></td>
                    <td>Mesa {{ $order->tableSession?->restaurantTable?->number ?? '—' }}</td>
            ...
            @empty
                <tr><td colspan="6" class="empty-state">No hay pedidos para mostrar.</td></tr>
            @endforelse
            </tbody>
        </table></div>
    </section>
