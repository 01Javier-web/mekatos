...
<div class="page-shell page-shell-narrow">
    ...
    <div class="order-detail-grid">
        ...
        <section class="panel"><div class="panel-header"><h3>Resumen</h3><span>{{ $order->orderItems->sum('quantity') }} unidades</span></div><div class="detail-body">
            ...
            @if ($order->notes)<div class="info-box" style="margin-top:16px;"><strong>Notas:</strong><br>{{ $order->notes }}</div>@endif
    ...
    <section class="panel history-panel"><div class="panel-header"><h3>Historial de estados</h3></div><div class="detail-body">@forelse ($order->statusHistories as $history)<div class="history-item"><strong>{{ $history->new_status }}</strong><span>{{ $history->changed_at?->format('d/m/Y H:i') }}{{ $history->changedBy ? ' · '.$history->changedBy->name : '' }}</span></div>@empty<p class="muted">Sin historial disponible.</p>@endforelse</div></section>
</div>
@endsection
