...
<div class="page-shell page-shell-narrow">
    <div class="page-heading"><div><span class="eyebrow">Administración</span><h2>Nuevo producto</h2><p>Agrega un producto al menú de Mekatos.</p></div><a class="button" href="{{ route('admin.products.index') }}">← Volver</a></div>
    ...
    <form class="form-card product-form" method="POST" action="{{ route('admin.products.store') }}">
        @csrf
        <div class="form-section-title"><strong>Información básica</strong><span>Estos datos se mostrarán en el menú.</span></div>
        <div class="form-grid">
            <label><span>Nombre</span><input type="text" name="name" value="{{ old('name') }}" required maxlength="255" autofocus placeholder="Ej. Hamburguesa especial"></label>
            <label><span>Categoría</span><select name="category_id" required><option value="">Selecciona una categoría</option>@foreach ($categories as $category)<option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>@endforeach</select></label>
        </div>
        <div class="form-grid">
            <label><span>Precio</span><input type="number" name="price" value="{{ old('price') }}" min="0" step="0.01" required placeholder="0"></label>
            <label><span>Ruta de imagen <small>(opcional)</small></span><input id="image-path" type="text" name="image_path" value="{{ old('image_path') }}" maxlength="255" placeholder="images/producto.jpg"><small class="field-help">Ruta relativa dentro de <code>public/</code>.</small></label>
        </div>
        ...
        <div class="form-actions"><a class="button" href="{{ route('admin.products.index') }}">Cancelar</a><button class="button button-primary" type="submit" id="save-product">Guardar producto</button></div>
    </form>
</div>
