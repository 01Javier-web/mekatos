...
class OrderWorkflowTest extends TestCase
{
    ...
    public function test_admin_can_start_order_and_waiter_can_deliver_it(): void
    {
        $admin = $this->admin();
        $waiter = $this->waiter();
        $order = $this->order();

        $this->actingAs($admin)->put(route('admin.orders.status', $order), ['status' => OrderStatus::PREPARING->value])->assertRedirect(route('admin.orders.show', $order));
