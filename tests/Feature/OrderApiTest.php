...
class OrderApiTest extends TestCase
{
    ...
    {
        $category = Category::create(['name' => 'Pruebas', 'description' => 'Categoría de pruebas', 'sort_order' => 1, 'is_active' => true]);
        return Product::create(['category_id' => $category->id, 'name' => 'Producto de prueba', 'description' => 'Producto para pruebas automáticas', 'price' => $price, 'is_available' => true]);
    }
    ...
    {
        $product = $this->product();
        $this->postJson('/api/orders', ['type' => OrderType::TAKEAWAY->value, 'items' => [['product_id' => $product->id, 'quantity' => 1]]])->assertUnauthorized();
    }

    public function test_authenticated_takeaway_has_no_table_session(): void
    {
        $user = $this->waiter(); $product = $this->product(12000);
        $response = $this->actingAs($user, 'sanctum')->postJson('/api/orders', ['type' => OrderType::TAKEAWAY->value, 'items' => [['product_id' => $product->id, 'quantity' => 1]]]);
        $response->assertCreated()->assertJsonPath('order.type', OrderType::TAKEAWAY->value)->assertJsonPath('order.table_session_id', null)->assertJsonPath('order.total', '12000.00');
    }
